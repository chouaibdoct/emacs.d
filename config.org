...
#+begin_src emacs-lisp :tangle yes
  ...
  ;; (defvar my-cpu-architecture "broadwell")
...
#+begin_src emacs-lisp :tangle yes
  (use-package whole-line-or-region
    ...
    :hook (prog-mode . whole-line-or-region-local-mode))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
  ...
  (use-package expreg
    ...
    :bind (("C-M-SPC" . expreg-expand)
  	 ("M--" . expreg-contract))
    :config
    (set 'expreg-restore-point-on-quit nil)
    )
#+end_src
* dired-native
#+begin_src emacs-lisp :tangle yes
  (use-package dired
    :ensure nil ; Built-in package, do not download
    :defer t
    :config
    ;; -a: Show all files (including hidden ones)
    ;; -g: Omit owner column (cleaner layout)
    ;; -h: Human-readable file sizes (e.g., 4k instead of 4096)
    ;; --group-directories-first: Group folders above files
    (setq dired-listing-switches "-agh --group-directories-first")
    
    (setq dired-dwim-target t)
    ;; :hook (dired-mode . dired-hide-details-mode) ;; "(" to toggle
...
#+begin_src emacs-lisp :tangle yes
  ...
  (use-package dired-subtree
    ...
    (:map dired-mode-map
  	...
  	("C-<up>" . dired-subtree-up)
  	("<backtab>" . dired-subtree-cycle)
  	)
    )

    ;; Note: Run `M-x nerd-icons-install-fonts` once after installing this!
...
#+begin_src emacs-lisp :tangle yes
  (use-package xenops
    ...
    (setq xenops-image-type 'svg)
    (setq xenops-update-delay 0.3)
...
#+begin_src emacs-lisp :tangle yes
  ;; (add-to-list 'native-comp-jit-compilation-deny-list "doom-modeline")
...
#+begin_src emacs-lisp :tangle yes
  (use-package eldoc
    ...
    :diminish
    :config
    (defun eldoc-and-follow ()
      (interactive)
      (eldoc-doc-buffer)
      (other-window 1)
      )
    :bind (("M-h" . eldoc-and-follow)
  	 ("C-c h" . eldoc)
  	 )
    )
#+end_src
  ...
  (use-package cmake-ts-mode
    ...
    (with-eval-after-load 'eglot
       (add-to-list 'eglot-server-programs
                   `(cmake-ts-mode . ,(eglot-alternatives
                                  '(("neocmakelsp" "--stdio")
...
#+begin_src emacs-lisp :tangle no
  ...
  ;;   :init
  ;;   (setq matlab-shell-command "/Applications/MATLAB_R2022b.app/bin/matlab")
...
#+begin_src emacs-lisp :tangle yes
  ...
  (use-package python
    ...
    :config
    (defun my/python-shell-after-send-defun (&rest _args)
      "Extension for python-shell-send-defun to prep the REPL call."
    ...
    ;; Apply the advice to the built-in function
    (advice-add 'python-shell-send-defun :after #'my/python-shell-after-send-defun)
    ;; (advice-remove 'python-shell-send-defun #'my/python-shell-after-send-defun) to revert
    )
  ...
  (use-package py-vterm-interaction
    ...
    :bind (
  	 :map py-vterm-interaction-mode-map
  	      ("C-c C-e" . py-vterm-interaction-send-region-or-current-line)
  	      ("C-C C-c" . py-vterm-interaction-send-buffer)
  	      )
    )
    ...
    (with-eval-after-load 'eglot
     (add-to-list 'eglot-server-programs
                 `(sql-mode . ,(eglot-alternatives
                                '(("/usr/local/bin/postgres-language-server-18" "lsp-proxy")
                                  ("/usr/local/bin/sqls"))))))
...
#+begin_src emacs-lisp :tangle yes
  (use-package pdf-tools
    ...
    (defun pdf-themed-on-themes()
      (when (seq-intersection '(modus-vivendi modus-vivendi-tinted ef-night ef-winter ef-dark) custom-enabled-themes)
...
#+begin_src emacs-lisp :tangle yes
  (use-package denote
    ...
    (setq denote-rename-buffer-format "%^s| %>40t")
